...
> {main}() called at [%s:2]
    > require_once("%s_require.inc") called at [%s:5]
        ...
        < basename("%s"...) = "trace_002_require.inc" called at [%s_require.inc:2] wt: %f
        > strlen("in trace_002_require.inc") called at [%s_require.inc:2]
        < strlen("in trace_002_require.inc") = 24 called at [%s_require.inc:2] wt: %f
    < require_once("%s_require.inc") = 1 called at [%s:5] wt: %f
    > include_once("%s_include.inc") called at [%s:6]
        > basename("%s"...) called at [%s_include.inc:2]
        < basename("%s"...) = "trace_002_include.inc" called at [%s_include.inc:2] wt: %f
        > strlen("in trace_002_include.inc") called at [%s_include.inc:2]
        < strlen("in trace_002_include.inc") = 24 called at [%s_include.inc:2] wt: %f
    < include_once("%s_include.inc") = 1 called at [%s:6] wt: %f
    > require("%s_require.inc") called at [%s:7]
        > basename("%s"...) called at [%s_require.inc:2]
        < basename("%s"...) = "trace_002_require.inc" called at [%s_require.inc:2] wt: %f
        > strlen("in trace_002_require.inc") called at [%s_require.inc:2]
        < strlen("in trace_002_require.inc") = 24 called at [%s_require.inc:2] wt: %f
    < require("%s_require.inc") = 1 called at [%s:7] wt: %f
    > include("%s_include.inc") called at [%s:8]
        > basename("%s"...) called at [%s_include.inc:2]
        < basename("%s"...) = "trace_002_include.inc" called at [%s_include.inc:2] wt: %f
        > strlen("in trace_002_include.inc") called at [%s_include.inc:2]
        < strlen("in trace_002_include.inc") = 24 called at [%s_include.inc:2] wt: %f
    < include("%s_include.inc") = 1 called at [%s:8] wt: %f
    > call_user_func("call_normal") called at [%s:11]
        > call_normal() called at [%s:11]
        < call_normal() = NULL called at [%s:11] wt: %f
    < call_user_func("call_normal") = NULL called at [%s:11] wt: %f
    > create_function("", "return 0;") called at [%s:14]
        > %r({main}|include|create_function)\(.*\)%r called at [%s:14]
        < %r({main}|include|create_function)\(.*\)%r = NULL called at [%s:14] wt: %f
    < create_function("", "return 0;") = "\x00lambda_1" called at [%s:14] wt: %f
    > {lambda:%s(14) : runtime-created function}() called at [%s:15]
    < {lambda:%s(14) : runtime-created function}() = 0 called at [%s:15] wt: %f
    > create_function("", "return 1;") called at [%s:16]
        > %r({main}|include|create_function)\(.*\)%r called at [%s:16]
        < %r({main}|include|create_function)\(.*\)%r = NULL called at [%s:16] wt: %f
    < create_function("", "return 1;") = "\x00lambda_2" called at [%s:16] wt: %f
    > {lambda:%s(16) : runtime-created function}() called at [%s:17]
    < {lambda:%s(16) : runtime-created function}() = 1 called at [%s:17] wt: %f
    > call_normal() called at [%s:21]
    < call_normal() = NULL called at [%s:21] wt: %f
    > {eval} called at [%s:24]
        > strlen("shit") called at [%s(24) : eval()'d code:1]
        < strlen("shit") = 4 called at [%s(24) : eval()'d code:1] wt: %f
        > call_normal() called at [%s(24) : eval()'d code:1]
        < call_normal() = NULL called at [%s(24) : eval()'d code:1] wt: %f
    < {eval} = NULL called at [%s:24] wt: %f
